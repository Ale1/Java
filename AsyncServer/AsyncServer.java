...
class AsyncServer {
	public static void main( String[] args) {
        ...
        try{
        	...
        	while (true){
        		// handle multiple connections by creating thread for each client connection
        		Thread thread = new Thread( new ConnectionHandler( serverSocket.accept() )  );
        		thread.start(); 
        	}
		 ...
		 try{
            ...
            String query = parseParams(in.readLine());                        				// (2)    get first line of headers, and extract url query param (e.g "ipod").
          	...
          	for( URL queryURL: queryURLS){  												// (4) for each country create requesthandler thread 
          		...
          		thread.start();																// (5) requestHandler does get and writes response asynchronously
          	}                           
